...
يشمل: أفضل 100 عملة، أخبار، ذهب، دولار، مؤشرات فنية، إدارة صفقات
"""
...
# إعدادات التداول (قابلة للتعديل)
CONFIG = {
    "min_volume_usdt": 500_000,      # الحد الأدنى للسيولة
    "min_volume_spike": 1.3,         # مضاعف الحجم
    "min_score": 50,                  # درجة AI المطلوبة (خفيفة حالياً)
    "cooldown_minutes": 10,           # راحة بين الإشارات
...
def get_binance_data(symbol, interval='15m', limit=300):
    try:
        ...
        print(f"خطأ جلب بيانات {symbol}: {e}")
        return pd.DataFrame()
...
# ========== الأخبار العالمية (ترامب، ماسك، حروب) ==========
def get_news():
    ...
    try:
        ...
        for entry in feed.entries[:15]:
            ...
            if "trump" in title:
                alerts["trump"] = title[:100]
            if "elon" in title or "musk" in title:
                alerts["musk"] = title[:100]
            if any(w in title for w in ["taiwan", "hormuz", "iran", "china", "war", "conflict"]):
                alerts["war"] = title[:100]
    except Exception as e:
        print(f"خطأ أخبار: {e}")
    return alerts
...
def send_signal(symbol, entry, tp1, tp2, sl, score, macro, news):
    msg = (
        f"🚀 *إشارة سكالبينغ متكاملة* 🚀\n\n"
        f"💎 *العملة:* {symbol}\n"
        f"🤖 *AI Score:* {score}/100\n"
        f"💰 *الدخول:* {entry:.6f}\n"
        f"🎯 *TP1:* {tp1:.6f} (+{(tp1/entry-1)*100:.2f}%)\n"
        f"🎯 *TP2:* {tp2:.6f} (+{(tp2/entry-1)*100:.2f}%)\n"
        f"🛑 *SL:* {sl:.6f} (-{(1-sl/entry)*100:.2f}%)\n\n"
        f"📊 *الاقتصاد الكلي:*\n"
        f"💵 الدولار: {macro['dollar']} | 🥇 الذهب: ${macro['gold']}\n\n"
        f"📰 *أخبار عاجلة:*\n"
        f"🇺🇸 ترامب: {news['trump'][:60]}\n"
        f"🚀 ماسك: {news['musk'][:60]}\n"
        f"⚔️ حروب: {news['war'][:60]}\n\n"
        f"⚠️ *إدارة المخاطرة: 1% من رأس المال*"
    )
    send_telegram(msg)
    active_trades[symbol] = {
        "entry": entry, "tp1": tp1, "tp2": tp2, "sl": sl,
        "open_time": datetime.now(), "tp1_hit": False, "closed": False
    }
    ...
    while True:
        for sym, trade in list(active_trades.items()):
            try:
                ...
                if price >= trade['tp2'] and not trade['closed']:
                    trade['closed'] = True
                    send_telegram(f"🏆 *{sym} حقق الهدف الثاني!*\nالربح: {profit*100:.2f}%")
                    del active_trades[sym]
                elif price <= trade['sl'] and not trade['closed']:
                    trade['closed'] = True
                    loss = (trade['entry']-price)/trade['entry']*100
                    send_telegram(f"🛑 *{sym} ضرب وقف الخسارة!*\nالخسارة: {loss:.2f}%")
                    del active_trades[sym]
                elif price >= trade['tp1'] and not trade['tp1_hit']:
                    trade['tp1_hit'] = True
                    send_telegram(f"🎯 *{sym} حقق الهدف الأول!*\nالربح: {profit*100:.2f}%")
            except:
                pass
        time.sleep(10)
...
def main():
    ...
    send_telegram("✅ *البوت المتكامل يعمل الآن*\n\n📊 يشمل: أفضل 100 عملة، أخبار ترامب وماسك، الذهب والدولار، مؤشرات فنية متقدمة، إدارة صفقات.\n\nسيتم الفحص التلقائي كل 30 دقيقة.")
